...
HR can ask natural questions — *"What happens if 20% of engineering takes leave next week?"* or *"What if 5 people resign?"* — and get computed answers: availability, workload redistribution, department capacity, and bottlenecks.

The design deliberately separates:

- **Layer 1 — Simulation Engine:** Pure, deterministic calculation logic over existing HRMS data. No ML. This is the layer that must be correct and explainable.
- **Layer 2 — Chatbot / NLP Front-End:** A thin wrapper around a locally hosted small language model (via **Ollama**, no third-party API calls) that parses the question and phrases the answer. Numbers are never generated by the model itself, keeping predictions fully auditable.
